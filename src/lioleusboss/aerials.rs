...
pub unsafe fn lioleusboss_air_howling_coroutine(item: &mut L2CAgentBase) -> L2CValue {
    ...
    MotionModule::change_motion(module_accessor,Hash40::new("air_howling"),0.0,1.0,false,0.0,false,false);
...
pub unsafe fn lioleusboss_low_fireball_shot_coroutine(item: &mut L2CAgentBase) -> L2CValue {
    let lua_state = item.lua_state_agent;
    let module_accessor = sv_system::battle_object_module_accessor(lua_state);
    MotionModule::change_motion(module_accessor,Hash40::new("low_fireball_shot"),0.0,1.0,false,0.0,false,false);
    boss_private::main_energy_from_param(lua_state,ItemKind(*ITEM_KIND_LIOLEUSBOSS),Hash40::new("energy_param_attack_fireball_air"),0.0);
    WorkModule::off_flag(module_accessor,*ITEM_LIOLEUSBOSS_INSTANCE_WORK_FLAG_SHOOT);
    return L2CValue::I32(0)
}

pub unsafe fn lioleusboss_low_fireball_shot_status(item: &mut L2CAgentBase) -> L2CValue {
    let lua_state = item.lua_state_agent;
    let module_accessor = sv_system::battle_object_module_accessor(lua_state);
    if WorkModule::is_flag(module_accessor,*ITEM_LIOLEUSBOSS_INSTANCE_WORK_FLAG_SHOOT) {
        ...
        if fireball.is_null() == false {
            action(fireball,*ITEM_LIOLEUSBOSSSHOT_ACTION_SET_ANGLE,-45.0);
        }
        WorkModule::off_flag(module_accessor,*ITEM_LIOLEUSBOSS_INSTANCE_WORK_FLAG_SHOOT);
    }
    if MotionModule::is_end(module_accessor) {
...
pub unsafe fn install_aerials(item: &mut L2CAgentBase) {
    ...
    let lioleusboss_low_fireball_shot_status_func: &mut skyline::libc::c_void = std::mem::transmute(L2CValue::Ptr(lioleusboss_low_fireball_shot_status as *const () as _).get_ptr());
    item.sv_set_status_func(L2CValue::I32(*ITEM_LIOLEUSBOSS_STATUS_KIND_ATTACK_FIREBALL_AIR),L2CValue::I32(*ITEM_LUA_SCRIPT_STATUS_FUNC_STATUS),lioleusboss_low_fireball_shot_status_func);
}