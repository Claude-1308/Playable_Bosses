...
pub unsafe fn entry_coroutine(item: &mut L2CAgentBase) -> L2CValue {
    ...
    if FighterUtil::is_hp_mode(owner) {
        let hp = WorkModule::get_float(owner,FIGHTER_MARIO_INSTANCE_WORK_ID_FLOAT_BOSS_HP) * Lioleusboss::extra_health_mul;
        set_hp(lua_state,hp);
        WorkModule::set_float(owner,hp,FIGHTER_MARIO_INSTANCE_WORK_ID_FLOAT_BOSS_HP_PREV);
    }
    else {
        set_hp(lua_state,Lioleusboss::health);
        WorkModule::set_float(owner,Lioleusboss::health,FIGHTER_MARIO_INSTANCE_WORK_ID_FLOAT_BOSS_HP_PREV);
    }
