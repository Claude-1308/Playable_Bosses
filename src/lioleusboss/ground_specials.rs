...
pub static mut LIOLEUSBOSS_TACKLE_JUMP_ENDING : usize = 0x4ad49c;

#[skyline::hook(replace = LIOLEUSBOSS_TACKLE_JUMP_ENDING, inline)]
pub unsafe fn lioleusboss_tackle_jump_ending(ctx: &mut InlineCtx) {
...
pub unsafe fn lioleusboss_tackle_jump_fix(fighter: &mut L2CFighterCommon) {
    let lua_state = fighter.lua_state_agent;
    let module_accessor = sv_system::battle_object_module_accessor(lua_state);
    let boss_id = WorkModule::get_int64(module_accessor,FIGHTER_MARIO_INSTANCE_WORK_ID_INT_BOSS_ID) as u32;
    let boss_boma = sv_battle_object::module_accessor(boss_id);
    if StatusModule::status_kind(boss_boma) == *ITEM_LIOLEUSBOSS_STATUS_KIND_ATTACK_TACKLE_JUMP {
        ...
        if AttackModule::is_attack(boss_boma,0,false) {
            AttackModule::set_power(boss_boma,0,5.0,false);
            AttackModule::set_poison_param(boss_boma,0,600,40,2.0,false);
            AttackModule::set_power(boss_boma,1,5.0,false);
            AttackModule::set_poison_param(boss_boma,1,600,40,2.0,false);
        }
    }
}

pub static mut LIOLEUSBOSS_FLIGHT_ENDING : usize = 0x4ad49c;

#[skyline::hook(replace = LIOLEUSBOSS_FLIGHT_ENDING, inline)]
pub unsafe fn lioleusboss_flight_ending(ctx: &mut InlineCtx) {
    let agent_base: &mut L2CAgentBase = std::mem::transmute(*ctx.registers[20].x.as_ref());
    if WorkModule::is_flag(agent_base.module_accessor,ITEM_INSTANCE_WORK_FLAG_PLAYER) == false {
        return;
    }
    WorkModule::set_int(agent_base.module_accessor,*SITUATION_KIND_AIR,FIGHTER_MARIO_INSTANCE_WORK_ID_INT_BOSS_SITUATION);
    *ctx.registers[0].w.as_mut() = *ITEM_LIOLEUSBOSS_STATUS_KIND_WAIT_AIR as u32;
}
