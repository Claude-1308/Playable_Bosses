...
pub unsafe fn install_teleport(item: &mut L2CAgentBase) {
    ...
    item.sv_set_status_func(L2CValue::I32(*ITEM_DARZ_STATUS_KIND_TELEPORT),L2CValue::I32(*ITEM_LUA_SCRIPT_STATUS_FUNC_STATUS_COROUTINE),teleport_coroutine_func);
    let teleport_status_func: &mut skyline::libc::c_void = std::mem::transmute(L2CValue::Ptr(teleport_status as *const () as _).get_ptr());
    item.sv_set_status_func(L2CValue::I32(*ITEM_DARZ_STATUS_KIND_TELEPORT),L2CValue::I32(*ITEM_LUA_SCRIPT_STATUS_FUNC_STATUS),teleport_status_func);
}
