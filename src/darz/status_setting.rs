...
pub static mut DARZ_TEAR_UP_SETTING : usize = 0x3ad310;

#[skyline::hook(replace = DARZ_TEAR_UP_SETTING)]
pub unsafe fn darz_tear_up_setting(item: &mut L2CAgentBase) -> L2CValue {
    ...
    if WorkModule::is_flag(module_accessor,ITEM_INSTANCE_WORK_FLAG_PLAYER) {
        ...
        if WorkModule::is_flag(owner,FIGHTER_MARIO_INSTANCE_WORK_ID_FLAG_ALLOT_STATUSES) == false {
            BossModule::install_moves(item,BossKind::DARZ);
...
pub unsafe fn darz_install_moves(item: &mut L2CAgentBase) {
