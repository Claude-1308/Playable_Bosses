...
use crate::darz::{move_input::*,normals::*};
...
pub unsafe fn entry_coroutine(item: &mut L2CAgentBase) -> L2CValue {
    ...
    if FighterUtil::is_hp_mode(owner) {
        ...
        set_hp(lua_state,Darz::health);
        WorkModule::set_float(owner,Darz::health,FIGHTER_MARIO_INSTANCE_WORK_ID_FLOAT_BOSS_HP_PREV);
        WorkModule::set_float(module_accessor,Darz::health,*ITEM_INSTANCE_WORK_FLOAT_HP_MAX);
    }
...
pub unsafe fn wait_coroutine(item: &mut L2CAgentBase) -> L2CValue {
    ...
    boss_private::main_energy_from_param(lua_state,ItemKind(*ITEM_KIND_DARZ),Hash40::new("energy_param_wait"),0.0);
...
pub unsafe fn wait_status(item: &mut L2CAgentBase) -> L2CValue {
    ...
    darz_move_inputs(owner,input_1,input_2);
    darz_movement(owner);
...
pub unsafe fn install_entry_dead_wait(item: &mut L2CAgentBase) {
    ...
    item.sv_set_status_func(L2CValue::I32(*ITEM_DARZ_STATUS_KIND_TEAR_UP),L2CValue::I32(*ITEM_LUA_SCRIPT_STATUS_FUNC_STATUS_COROUTINE),entry_coroutine_func);
    let entry_status_func: &mut skyline::libc::c_void = std::mem::transmute(L2CValue::Ptr(entry_status as *const () as _).get_ptr());
    item.sv_set_status_func(L2CValue::I32(*ITEM_DARZ_STATUS_KIND_TEAR_UP),L2CValue::I32(*ITEM_LUA_SCRIPT_STATUS_FUNC_STATUS),entry_status_func);
    let entry_end_func: &mut skyline::libc::c_void = std::mem::transmute(L2CValue::Ptr(entry_end as *const () as _).get_ptr());
    item.sv_set_status_func(L2CValue::I32(*ITEM_DARZ_STATUS_KIND_TEAR_UP),L2CValue::I32(*LUA_SCRIPT_STATUS_FUNC_STATUS_END),entry_end_func);
    
    let wait_coroutine_func: &mut skyline::libc::c_void = std::mem::transmute(L2CValue::Ptr(wait_coroutine as *const () as _).get_ptr());
    item.sv_set_status_func(L2CValue::I32(*ITEM_DARZ_STATUS_KIND_MANAGER_WAIT),L2CValue::I32(*ITEM_LUA_SCRIPT_STATUS_FUNC_STATUS_COROUTINE),wait_coroutine_func);
    ...
    item.sv_set_status_func(L2CValue::I32(*ITEM_DARZ_STATUS_KIND_MANAGER_WAIT),L2CValue::I32(*ITEM_LUA_SCRIPT_STATUS_FUNC_STATUS),wait_status_func);
