...
pub static mut DRACULA_CHANGE_SETTING : usize = 0x3e8950;

#[skyline::hook(replace = DRACULA_CHANGE_SETTING)]
pub unsafe fn dracula_change_setting(item: &mut L2CAgentBase) -> L2CValue {
    ...
    if WorkModule::is_flag(module_accessor,ITEM_INSTANCE_WORK_FLAG_PLAYER) {
        ...
        if WorkModule::is_flag(owner,FIGHTER_MARIO_INSTANCE_WORK_ID_FLAG_ALLOT_STATUSES) == false {
            BossModule::install_moves(item,BossKind::DRACULA);
...
pub unsafe fn dracula_install_moves(item: &mut L2CAgentBase) {
