...
pub unsafe fn teleport_status(item: &mut L2CAgentBase) -> L2CValue {
    ...
    if MotionModule::is_end(module_accessor) && MotionModule::motion_kind(module_accessor) == hash40("teleport_start") {
        let dir_mul = 100.0;
        let pos = Vector2f{x: ControlModule::get_stick_x(owner) * dir_mul, y: 0.0};
...
pub unsafe fn install_teleport(item: &mut L2CAgentBase) {
    ...
    item.sv_set_status_func(L2CValue::I32(*ITEM_DRACULA_STATUS_KIND_TELEPORT_START),L2CValue::I32(*ITEM_LUA_SCRIPT_STATUS_FUNC_STATUS_COROUTINE),teleport_coroutine_func);
    let teleport_status_func: &mut skyline::libc::c_void = std::mem::transmute(L2CValue::Ptr(teleport_status as *const () as _).get_ptr());
    item.sv_set_status_func(L2CValue::I32(*ITEM_DRACULA_STATUS_KIND_TELEPORT_START),L2CValue::I32(*ITEM_LUA_SCRIPT_STATUS_FUNC_STATUS),teleport_status_func);
}