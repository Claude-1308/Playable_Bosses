...
use crate::dracula::{move_input::*,specials::*};
...
pub unsafe fn entry_coroutine(item: &mut L2CAgentBase) -> L2CValue {
    ...
    if FighterUtil::is_hp_mode(owner) {
        ...
        set_hp(lua_state,Dracula::health);
        WorkModule::set_float(owner,Dracula::health,FIGHTER_MARIO_INSTANCE_WORK_ID_FLOAT_BOSS_HP_PREV);
        WorkModule::set_float(owner,Dracula::health,FIGHTER_MARIO_INSTANCE_WORK_ID_FLOAT_BOSS_HP);
    }
    ...
    MotionModule::change_motion(module_accessor,Hash40::new("entry"),300.0,1.0,false,0.0,false,false);
...
pub unsafe fn entry_status(item: &mut L2CAgentBase) -> L2CValue {
    ...
    if MotionModule::is_end(module_accessor) {
        let dracula_change_start_coroutine_func: &mut skyline::libc::c_void = std::mem::transmute(L2CValue::Ptr(dracula_change_start_coroutine as *const () as _).get_ptr());
        item.sv_set_status_func(L2CValue::I32(*ITEM_DRACULA_STATUS_KIND_CHANGE_START),L2CValue::I32(*ITEM_LUA_SCRIPT_STATUS_FUNC_STATUS_COROUTINE),dracula_change_start_coroutine_func);
        let dracula_change_start_status_func: &mut skyline::libc::c_void = std::mem::transmute(L2CValue::Ptr(dracula_change_start_status as *const () as _).get_ptr());
        item.sv_set_status_func(L2CValue::I32(*ITEM_DRACULA_STATUS_KIND_CHANGE_START),L2CValue::I32(*ITEM_LUA_SCRIPT_STATUS_FUNC_STATUS),dracula_change_start_status_func);
        StatusModule::change_status_request(module_accessor,*ITEM_DRACULA_STATUS_KIND_WAIT,false);
    }
    return L2CValue::I32(0)
}
...
pub unsafe fn wait_coroutine(item: &mut L2CAgentBase) -> L2CValue {
    ...
    boss_private::main_energy_from_param(lua_state,ItemKind(*ITEM_KIND_DRACULA),Hash40::new("energy_param_wait"),0.0);
...
pub unsafe fn wait_status(item: &mut L2CAgentBase) -> L2CValue {
    ...
    dracula_move_inputs(owner,input_1,input_2);
...
pub unsafe fn install_entry_dead_wait(item: &mut L2CAgentBase) {
    ...
    item.sv_set_status_func(L2CValue::I32(*ITEM_DRACULA_STATUS_KIND_CHANGE_START),L2CValue::I32(*ITEM_LUA_SCRIPT_STATUS_FUNC_STATUS_COROUTINE),entry_coroutine_func);
    let entry_status_func: &mut skyline::libc::c_void = std::mem::transmute(L2CValue::Ptr(entry_status as *const () as _).get_ptr());
    item.sv_set_status_func(L2CValue::I32(*ITEM_DRACULA_STATUS_KIND_CHANGE_START),L2CValue::I32(*ITEM_LUA_SCRIPT_STATUS_FUNC_STATUS),entry_status_func);
    
    let wait_coroutine_func: &mut skyline::libc::c_void = std::mem::transmute(L2CValue::Ptr(wait_coroutine as *const () as _).get_ptr());
    item.sv_set_status_func(L2CValue::I32(*ITEM_DRACULA_STATUS_KIND_WAIT),L2CValue::I32(*ITEM_LUA_SCRIPT_STATUS_FUNC_STATUS_COROUTINE),wait_coroutine_func);
    ...
    item.sv_set_status_func(L2CValue::I32(*ITEM_DRACULA_STATUS_KIND_WAIT),L2CValue::I32(*ITEM_LUA_SCRIPT_STATUS_FUNC_STATUS),wait_status_func);
