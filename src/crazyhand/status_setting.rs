...
pub static mut CH_WAIT_TIME_SETTING : usize = 0x379700;

#[skyline::hook(replace = CH_WAIT_TIME_SETTING)]
pub unsafe fn ch_wait_time_setting(item: &mut L2CAgentBase) -> L2CValue {
    ...
    if WorkModule::is_flag(module_accessor,ITEM_INSTANCE_WORK_FLAG_PLAYER) {
        ...
        if WorkModule::is_flag(owner,FIGHTER_MARIO_INSTANCE_WORK_ID_FLAG_ALLOT_STATUSES) == false {
            BossModule::install_moves(item,BossKind::CRAZYHAND);
...
pub unsafe fn ch_install_moves(item: &mut L2CAgentBase) {
    install_entry_dead_wait(item);
    install_normals(item);
    install_ground_specials(item);
    install_air_specials(item);
    install_super_specials(item);
    install_teleport_turn(item);
}