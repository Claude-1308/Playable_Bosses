...
pub unsafe fn turn_pre(item: &mut L2CAgentBase) -> L2CValue {
    ...
    boss_private::main_energy_from_param(lua_state,ItemKind(*ITEM_KIND_CRAZYHAND),Hash40::new("energy_param_turn"),0.0);
    return L2CValue::I32(0)
}
...
pub unsafe fn install_teleport_turn(item: &mut L2CAgentBase) {
    ...
    item.sv_set_status_func(L2CValue::I32(*ITEM_CRAZYHAND_STATUS_KIND_WAIT_TELEPORT),L2CValue::I32(*LUA_SCRIPT_STATUS_FUNC_STATUS_PRE),teleport_pre_func);
    ...
    item.sv_set_status_func(L2CValue::I32(*ITEM_CRAZYHAND_STATUS_KIND_WAIT_TELEPORT),L2CValue::I32(*ITEM_LUA_SCRIPT_STATUS_FUNC_STATUS_COROUTINE),teleport_coroutine_func);
    let teleport_status_func: &mut skyline::libc::c_void = std::mem::transmute(L2CValue::Ptr(teleport_status as *const () as _).get_ptr());
    item.sv_set_status_func(L2CValue::I32(*ITEM_CRAZYHAND_STATUS_KIND_WAIT_TELEPORT),L2CValue::I32(*ITEM_LUA_SCRIPT_STATUS_FUNC_STATUS),teleport_status_func);
    
    let turn_pre_func: &mut skyline::libc::c_void = std::mem::transmute(L2CValue::Ptr(turn_pre as *const () as _).get_ptr());
    item.sv_set_status_func(L2CValue::I32(*ITEM_CRAZYHAND_STATUS_KIND_TURN),L2CValue::I32(*LUA_SCRIPT_STATUS_FUNC_STATUS_PRE),turn_pre_func);
    let turn_coroutine_func: &mut skyline::libc::c_void = std::mem::transmute(L2CValue::Ptr(turn_coroutine as *const () as _).get_ptr());
    item.sv_set_status_func(L2CValue::I32(*ITEM_CRAZYHAND_STATUS_KIND_TURN),L2CValue::I32(*ITEM_LUA_SCRIPT_STATUS_FUNC_STATUS_COROUTINE),turn_coroutine_func);
    let turn_status_func: &mut skyline::libc::c_void = std::mem::transmute(L2CValue::Ptr(turn_status as *const () as _).get_ptr());
    item.sv_set_status_func(L2CValue::I32(*ITEM_CRAZYHAND_STATUS_KIND_TURN),L2CValue::I32(*ITEM_LUA_SCRIPT_STATUS_FUNC_STATUS),turn_status_func);
}