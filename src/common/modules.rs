pub mod BossModule {
    ...
    use crate::dracula::{*,status_setting::*};
    use crate::dracula2::{*,status_setting::*};
    use crate::darz::{*,status_setting::*};
    use crate::kiila::{*,status_setting::*};

    pub unsafe fn allot_boss(fighter_boma: &mut BattleObjectModuleAccessor, name: u64) {
        let mut boss_kind = -1;
        if name == hash40("ui_chara_masterhand") {
            boss_kind = BossKind::MASTERHAND;
        }
        else if name == hash40("ui_chara_crazyhand") {
            boss_kind = BossKind::CRAZYHAND;
        }
        else if name == hash40("ui_chara_ganonboss") {
            boss_kind = BossKind::GANONBOSS;
        }
        else if name == hash40("ui_chara_lioleus") {
            boss_kind = BossKind::LIOLEUSBOSS;
        }
        else if name == hash40("ui_chara_galleom") {
            boss_kind = BossKind::GALLEOM;
        }
        else if name == hash40("ui_chara_marx") {
            boss_kind = BossKind::MARX;
        }
        else if name == hash40("ui_chara_dracula") {
            boss_kind = BossKind::DRACULA;
        }
        else if name == hash40("ui_chara_darz") {
            boss_kind = BossKind::DARZ;
        }
        else if name == hash40("ui_chara_kiila") {
            boss_kind = BossKind::KIILA;
        }
    ...
    pub unsafe fn summon_boss(fighter_boma: &mut BattleObjectModuleAccessor) {
        ...
        let kind = match boss {
            ...
            BossKind::DRACULA => *ITEM_KIND_DRACULA,
            BossKind::DRACULA2 => *ITEM_KIND_DRACULA2,
            BossKind::DARZ => *ITEM_KIND_DARZ,
            BossKind::KIILA => *ITEM_KIND_KIILA,
            _ => 0
        };
        ...
        if FighterUtil::is_hp_mode(fighter_boma) {
            ...
            if kind == BossKind::DRACULA2 {
                WorkModule::set_float(fighter_boma,player_hp.ceil() * 2.0,FIGHTER_MARIO_INSTANCE_WORK_ID_FLOAT_BOSS_HP);
            }
            else {
        ...
        match boss {
            ...
            BossKind::DRACULA => {
                StatusModule::change_status_request(boss_boma,*ITEM_DRACULA_STATUS_KIND_CHANGE_START,false);
            },
            BossKind::DRACULA2 => {
                WorkModule::on_flag(fighter_boma,FIGHTER_MARIO_INSTANCE_WORK_ID_FLAG_PLAY_ENTRY);
                WorkModule::off_flag(fighter_boma,FIGHTER_MARIO_INSTANCE_WORK_ID_FLAG_ALLOT_STATUSES);
                StatusModule::change_status_request(boss_boma,*ITEM_DRACULA2_STATUS_KIND_WAIT,false);
            },
            BossKind::DARZ => {
                StatusModule::change_status_request(boss_boma,*ITEM_DARZ_STATUS_KIND_TEAR_UP,false);
            },
            BossKind::KIILA => {
                StatusModule::change_status_request(boss_boma,*ITEM_KIILA_STATUS_KIND_TORRENT,false);
            },
            _ => {}
        };
    }
    ...
    pub unsafe fn install_moves(item: &mut L2CAgentBase, kind: i32) {
        match kind {
            ...
            BossKind::DRACULA => dracula_install_moves(item),
            BossKind::DRACULA2 => dracula2_install_moves(item),
            BossKind::DARZ => darz_install_moves(item),
            BossKind::KIILA => kiila_install_moves(item),
            _ => {}
        };
    }
}