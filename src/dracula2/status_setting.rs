...
pub static mut DRACULA2_WAIT_SETTING : usize = 0x4001e0;

#[skyline::hook(replace = DRACULA2_WAIT_SETTING)]
pub unsafe fn dracula2_wait_setting(item: &mut L2CAgentBase) -> L2CValue {
    ...
    if WorkModule::is_flag(module_accessor,ITEM_INSTANCE_WORK_FLAG_PLAYER) {
        ...
        if WorkModule::is_flag(owner,FIGHTER_MARIO_INSTANCE_WORK_ID_FLAG_ALLOT_STATUSES) == false {
            BossModule::install_moves(item,BossKind::DRACULA2);
...
pub unsafe fn dracula2_install_moves(item: &mut L2CAgentBase) {
    install_entry_dead_wait(item);
    install_normals(item);
    install_specials(item);
    install_turn_specials(item);
}