...
use crate::masterhand::*;

pub static mut MH_WAIT_TIME_SETTING : usize = 0x54cd90;

#[skyline::hook(replace = MH_WAIT_TIME_SETTING)]
pub unsafe fn mh_wait_time_setting(item: &mut L2CAgentBase) -> L2CValue {
    ...
    if WorkModule::is_flag(module_accessor,ITEM_INSTANCE_WORK_FLAG_PLAYER) {
        ...
        if WorkModule::is_flag(owner,FIGHTER_MARIO_INSTANCE_WORK_ID_FLAG_ALLOT_STATUSES) == false {
            BossModule::install_moves(item,BossKind::MASTERHAND);
...
pub unsafe fn mh_install_moves(item: &mut L2CAgentBase) {
